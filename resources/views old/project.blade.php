...
@section('title'){{$meta_title ?? null}} @endsection

@section('description') {{$meta_description ?? null}} @endsection
@section('keywords') {{$keywords ?? null}} @endsection
...
<div class="relative">
    ...
    <div class="grid grid-cols-1">
                     <div class="container  z-[2] pl-[60px] lg:pl-[50px] 2xl::pl-[0px] lg:absolute lg:left-0 lg:right-0 md:mt-[5rem] mt-[1rem] lg:translate-y-[-320px]" id="filterz">
                            ...
                            <div class="col-span-12">
                                <div class="relative selectricc-border-none ">
                                    <button class="tab-toggle-btn px-[10px] py-[15px] absolute top-[0px] left-[-45px] border-l border-t border-b border-solid border-[#016450] bg-[rgb(236,250,247,0.8)] text-primary rounded-tl-[10px] rounded-bl-[10px]" id="b1" aria-label="svg icon">
                                        ...
                                        <div id="co-living" class="active tab-content bg-[rgb(237,252,250,0.9)] border border-solid border-[#016450] rounded-bl-[15px] rounded-br-[15px] rounded-tr-[15px] px-[15px] sm:px-[30px] py-[28px] lg:max-h-56 overflow-visible">
                    ...
                    <div class="overflow-hidden rounded-md drop-shadow-[0px_2px_15px_rgba(0,0,0,0.1)] bg-[#FFFDFC] text-center transition-all duration-300 hover:-translate-y-[10px]">
                        <div class="relative">
                            <a href="{{ route('project_details',$project->id) }}" class="block">
                                <img src="{{config('fgproperties.file_path').$project->featured_image}}" class="w-full h-[15rem]" loading="lazy" width="370" height="266" alt="Sobha Creek Vistas Tower C.">
                            </a>
                        ...
                        <div class="py-[20px] px-[20px]">
                            <h3><a href="{{ route('project_details',$project->id) }}" class="font-lora leading-tight text-[22px] xl:text-[26px] text-primary font-medium">{{$project->title}}</a></h3>
                            <h4><a href="{{ route('project_details',$project->id) }}" class="font-light text-tiny underline">
                                @if($project->url != "")
                                    {{$project->url}}
                                @else
                                   UAE
                            ...
                            <span class="font-light text-sm">Added: {{ Carbon\Carbon::parse($project->created_at)->format('d-M-Y') }}</span>
                            ...
                            <!-- <p class="font-light">{{$project->area}} Sq.fit - {{$project->bedrooms}} Bed - {{$project->bathrooms}} Bath</p> -->
                            <p class="font-light">
                                @if($project->area != "")
                                    {{$project->area}} Sq.fit -
                                @else
